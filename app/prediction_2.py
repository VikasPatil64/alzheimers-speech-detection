...
def predict_from_audio_file_with_clinical(audio_path, clinical_data=None, provided_transcript=None):
    ...
    if filler_feat['filler_rate'] > 0.10 and pred == 0:  # >10% filler words
        override = True
    ...
    # CHECK 1: Young, educated, high MMSE speakers should NOT get Dementia
    if age_raw < 60 and mmse_raw > 26 and education_raw > 14:
        if pred == 1:  # Model said Dementia
            print("   ⚠️ SAFETY: Young, educated, high MMSE speaker – overriding Dementia prediction")
            pred = 0  # Override to Healthy
            prob[0] = 0.85
            prob[1] = 0.15
            override = True
            override_reason = "Young age, high education, and normal MMSE indicate low risk."
    
    # CHECK 2: High word count + good fluency = Healthy
    if word_count > 120 and words_per_minute > 140:
        if pred == 1:  # Model said Dementia but speaker is fluent
            print(f"   ⚠️ SAFETY: High word count ({word_count}) and normal speech rate ({words_per_minute:.0f} wpm)")
            pred = 0
            prob[0] = max(prob[0], 0.80)
            override = True
            override_reason = "Normal speech rate and word count indicate healthy speaker."
    
    # CHECK 3: Normal MMSE + Good word count = Healthy
    if mmse_raw >= 26 and word_count > 80:
        if pred == 1:
            print(f"   ⚠️ SAFETY: Normal MMSE ({mmse_raw}) and adequate word count ({word_count})")
            pred = 0
            prob[0] = max(prob[0], 0.75)
            override = True
            override_reason = "Normal cognitive score and adequate speech output."
    
    # CHECK 4: Reduce confidence for ambiguous cases (don't override, just lower confidence)
    # If speaker is young but model is highly confident, reduce confidence
    if age_raw < 65 and pred == 1 and prob[1] > 0.85:
        prob[1] = prob[1] * 0.7
        print(f"   ⚠️ Reduced confidence for young speaker (age {age_raw})")
    
    # If education is high but model predicts Dementia, reduce confidence
    if education_raw > 16 and pred == 1 and prob[1] > 0.80:
        prob[1] = prob[1] * 0.8
